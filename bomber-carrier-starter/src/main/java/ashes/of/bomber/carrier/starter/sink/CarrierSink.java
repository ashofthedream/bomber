...
public class CarrierSink implements Sink {
    ...
    @Override
    public void startUp() {
        SinkEvent event = event(TEST_APP_START, Instant.now(), null, null);
        send(event);
    }

    @Override
    public void beforeTestSuite(Stage stage, String testSuite, Instant timestamp, Settings settings) {
        SinkEvent event = event(TEST_SUITE_START, timestamp, testSuite, null);
        send(event);
    }
    ...
    public void beforeTestCase(Stage stage, String testSuite, String testCase, Instant timestamp, Settings settings) {
        SinkEvent event = event(TEST_CASE_START, timestamp, testSuite, testCase);
        send(event);
    }

    @Override
    public void afterTestCase(Stage stage, String testSuite, String testCase) {
        SinkEvent event = event(TEST_CASE_FINISH, Instant.now(), testSuite, testCase);
        send(event);
    }

    @Override
    public void afterTestSuite(Stage stage, String testSuite) {
        SinkEvent event = event(TEST_SUITE_FINISH, Instant.now(), testSuite, null);
        send(event);
    }

    @Override
    public void shutDown() {
        SinkEvent event = event(TEST_APP_FINISH, Instant.now(), null, null);
        send(event);
    }
    ...
    private SinkEvent event(SinkEventType type, Instant timestamp, @Nullable String testSuite, @Nullable String testCase) {
        return SinkEvent.builder()
                .type(type)
                .timestamp(timestamp.toEpochMilli())
                .flightId(app.getFlightId())
